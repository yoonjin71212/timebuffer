...
int8_t between(list* lst, char* item, node* b, node* n, uint64_t len, uint64_t t_stamp)
{
    ...
    if((b!=n->prev)) { //you can't push your element when extra nodes are in between of two nodes.
        return -1;
    } else if ((element=(node *) kvmalloc(sizeof(node),GFP_USER))==NULL) {
        return 1;
    } else {
...
{
    node* push_node=kvmalloc(sizeof(node),GFP_USER);
    node* target_node=kvmalloc(sizeof(node),GFP_USER);
    node* target_end=kvmalloc(sizeof(node),GFP_USER);
